...
# Script to run CARLA Driving Simulator Client in development mode
# Usage: ./scripts/docker/run-dev.sh [backend|frontend|full]
...
echo "Starting CARLA Driving Simulator Client in development mode..."
...
case $SERVICE in
    "backend")
        COMPOSE_FILE="$COMPOSE_DIR/docker-compose-backend-dev.yml"
        echo "Running backend only..."
        ;;
    "frontend")
        COMPOSE_FILE="$COMPOSE_DIR/docker-compose-frontend-dev.yml"
        echo "Running frontend only..."
        ;;
    "full"|*)
        COMPOSE_FILE="$COMPOSE_DIR/docker-compose-full-dev.yml"
