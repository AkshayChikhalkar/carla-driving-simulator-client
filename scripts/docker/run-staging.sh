...
# Script to run CARLA Driving Simulator Client in staging mode
# Usage: ./scripts/docker/run-staging.sh [backend|frontend|full]
...
echo "Starting CARLA Driving Simulator Client in staging mode..."
...
case $SERVICE in
    "backend")
        COMPOSE_FILE="$COMPOSE_DIR/docker-compose-backend-staging.yml"
        echo "Running backend only..."
        ;;
    "frontend")
        COMPOSE_FILE="$COMPOSE_DIR/docker-compose-frontend-staging.yml"
        echo "Running frontend only..."
        ;;
    "full"|*)
        COMPOSE_FILE="$COMPOSE_DIR/docker-compose-full-staging.yml"
