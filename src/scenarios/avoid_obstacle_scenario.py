...
class AvoidObstacleScenario(BaseScenario):
    ...
    def __init__(self, 
        ...
        self.obstacle_types = config.get('obstacle_types', ["static.prop.streetbarrier01"])
        ...
        self._current_loc = carla.Location()  # Pre-allocate location for distance calculations
        self.start_time = 0.0
    ...
    def find_alternative_path(self, current_loc: carla.Location, target_loc: carla.Location) -> Optional[carla.Location]:
        ...
        try:
            ...
            angles = [-self.avoidance_angle, self.avoidance_angle, -self.avoidance_angle/2, self.avoidance_angle/2]
            ...
            for angle in angles:
                ...
                alt_x = current_loc.x + self.avoidance_distance * (forward_vector.x * math.cos(alt_angle) - forward_vector.y * math.sin(alt_angle))
                alt_y = current_loc.y + self.avoidance_distance * (forward_vector.x * math.sin(alt_angle) + forward_vector.y * math.cos(alt_angle))
            ...
            if best_alt_waypoint and max_clear_distance > self.obstacle_detection_range / 2:
    ...
    def update(self) -> None:
        ...
        try:
            ...
            if not self.scenario_started and self.current_speed > 5.0:  # Wait until vehicle reaches 5 km/h
                ...
                for obstacle in self.obstacles:
                    ...
                    if distance_to_obstacle < self.emergency_brake_distance:
                        ...
                        if obstacle.id not in self.logged_obstacles:
                            self.logger.debug(f"Emergency brake triggered! Distance to obstacle: {distance_to_obstacle:.2f}m")
                    ...
                    if distance_to_obstacle < self.obstacle_detection_range:
                        ...
                        if obstacle.id not in self.logged_obstacles:
                            self.logger.info(f"Obstacle detected at distance: {distance_to_obstacle:.2f}m")
                            self.logged_obstacles.add(obstacle.id)
                
                # If we have a current avoidance target, check if we should continue avoiding
                if self.current_avoidance_target:
                    distance_to_avoidance = self._current_loc.distance(self.current_avoidance_target)
                    if distance_to_avoidance < self.waypoint_tolerance:
                        self.current_avoidance_target = None
                        self.logger.info("Reached avoidance target, returning to original path")
                    else:
                        # Continue following avoidance target at reduced speed
                        self.vehicle_controller.set_target(self.current_avoidance_target)
                    ...
                    if closest_obstacle:
                        ...
                        if alternative_target:
                            ...
                            if closest_obstacle.id not in self.logged_obstacles:
                                self.logger.info(f"Taking alternative path to avoid obstacle at {closest_obstacle.get_location()}")
