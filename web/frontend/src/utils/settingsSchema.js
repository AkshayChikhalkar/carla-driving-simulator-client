...
// Each field can include an optional description to be shown as a tooltip/secondary text
// App settings schema (UI, auth, analytics, etc.)
export const APP_SETTINGS_CATEGORIES = [
  {
    key: 'server',
    label: 'Server',
    fields: [
      { path: 'server.host', label: 'Host', type: 'string', description: 'CARLA server hostname or IP' },
      { path: 'server.port', label: 'Port', type: 'number', min: 1, max: 65535, description: 'CARLA server TCP port' },
      { path: 'server.timeout', label: 'Timeout (s)', type: 'number', min: 1, step: 0.1, description: 'Connection timeout in seconds' },
      { path: 'server.connection.max_retries', label: 'Max Retries', type: 'number', min: 0, description: 'Maximum reconnect attempts' },
      { path: 'server.connection.retry_delay', label: 'Retry Delay (s)', type: 'number', min: 0, step: 0.1, description: 'Delay between reconnects' },
    ],
  },
  {
    key: 'display',
    label: 'Display',
    fields: [
      { path: 'display.width', label: 'Width', type: 'number', description: 'Render width in pixels' },
      { path: 'display.height', label: 'Height', type: 'number' },
      { path: 'display.fps', label: 'FPS', type: 'number' },
      { path: 'display.hud_enabled', label: 'HUD', type: 'boolean' },
      { path: 'display.minimap_enabled', label: 'Minimap', type: 'boolean' },
    ],
  },
  {
    key: 'analytics',
...
// Simulation settings schema (CARLA-related)
export const SIM_SETTINGS_CATEGORIES = [
  {
    key: 'simulation',
    label: 'Simulation',
    fields: [
      { path: 'simulation.max_speed', label: 'Max Speed (km/h)', type: 'number', description: 'Upper bound for vehicle speed' },
  ...
  },
  {
    ...
    label: 'World',
    fields: [
      { path: 'world.map', label: 'Map', type: 'enum', optionsFromCatalog: 'maps', description: 'Town to load' },
      { path: 'world.walkers.blueprint', label: 'Walker Blueprint', type: 'enum', optionsFromCatalog: 'walkers', description: 'Default walker blueprint' },
      { path: 'world.walkers.count', label: 'Walker Count', type: 'number', min: 0 },
  ...
  },
  {
    ...
    fields: [
      ...
      // dynamic enums from catalog
      { path: 'sensors.primary', label: 'Primary Sensor', type: 'enum', optionsFromCatalog: 'sensors', description: 'Select a sensor blueprint' },
    ],
  },
  {
    key: 'controller',
    label: 'Controller',
    fields: [
      { path: 'controller.type', label: 'Controller Type', type: 'enum', options: ['autopilot', 'keyboard', 'gamepad'], description: 'Select how the vehicle is controlled' },
  ...
  },
  {
    ...
    label: 'Vehicle',
    fields: [
      { path: 'vehicle.model', label: 'Model', type: 'enum', optionsFromCatalog: 'vehicles', description: 'Vehicle blueprint id' },
  ...
  {
    key: 'advanced',
    label: 'Advanced (CARLA Blueprints)',
    fields: [
      { path: 'advanced.vehicle_blueprint', label: 'Vehicle Blueprint', type: 'enum', optionsFromCatalog: 'vehicles', description: 'Choose a vehicle blueprint to edit attributes' },
      { path: 'advanced.sensor_blueprint', label: 'Sensor Blueprint', type: 'enum', optionsFromCatalog: 'sensors', description: 'Choose a sensor blueprint to edit attributes' },
      { path: 'advanced.walker_blueprint', label: 'Walker Blueprint', type: 'enum', optionsFromCatalog: 'walkers', description: 'Choose a walker blueprint to edit attributes' },
    ],
  },
];
